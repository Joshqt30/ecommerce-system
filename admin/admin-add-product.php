...
try {
    $pdo = new PDO($dsn, 'postgres', 'Joshpogi123', [
...
<head>
  ...
  .sidebar-logo-icon svg { width: 20px; height: 20px; }
    ...
    /* Full active: current page owns this section */
    .nav-item.active { background: var(--text); color: #fff; font-weight: 600; }
    .nav-item.active svg { color: #fff; }
    /* Parent-active: this page is a child of that section — subtle highlight, NOT dark pill */
    .nav-item.parent-active { background: rgba(0,0,0,0.06); font-weight: 600; }

    /* ── Main ───────────────────────────────────────── */
    .main-content { display: flex; flex-direction: column; padding: 28px 28px 48px; gap: 0; overflow-y: auto; }
    ...
    .form-label { display: block; font-size: 12px; font-weight: 600; color: var(--text); margin-bottom: 7px; }
    .form-label .req { color: var(--red); margin-left: 2px; }
    .form-label .opt { color: var(--muted); font-weight: 400; margin-left: 4px; }

    .form-input, .form-textarea, .form-select {
      width: 100%; font-family: var(--font); font-size: 13px; color: var(--text);
      background: #fafafa; border: 1.5px solid var(--border); border-radius: 10px;
    ...
    .input-prefix {
      ...
      overflow: hidden; background: #fafafa; transition: border-color .18s;
    }
    .input-prefix:focus-within { border-color: #888; background: #fff; }
    .input-prefix-label {
      display: flex; align-items: center; padding: 0 12px;
      background: #f0f0f0; border-right: 1.5px solid var(--border);
    ...
    /* Discount toggle */
    .discount-toggle-row { display: flex; align-items: center; justify-content: space-between; margin-bottom: 12px; }
    .toggle-switch { position: relative; display: inline-block; width: 40px; height: 22px; }
    .toggle-switch input { opacity: 0; width: 0; height: 0; }
    .toggle-slider {
      position: absolute; inset: 0; background: #d1d5db; border-radius: 22px; cursor: pointer; transition: background .2s;
    }
    ...
    .drop-icon {
      width: 44px; height: 44px; margin: 0 auto 12px; background: #f0f0f0;
    ...
    .preview-thumb img { width: 100%; height: 100%; object-fit: cover; border-radius: 8px; border: 1.5px solid var(--border); }
    .remove-img {
    ...
    .tags-wrap {
      ...
      background: #fafafa; cursor: text; transition: border-color .18s;
    }
    .tags-wrap:focus-within { border-color: #888; background: #fff; }
    .tag-chip {
      display: flex; align-items: center; gap: 5px; padding: 3px 10px;
      background: #e0f2fe; color: #0369a1; border-radius: 20px; font-size: 11px; font-weight: 600;
    }
    .tag-chip button {
    ...
    .tag-input {
      ...
      font-size: 12px; color: var(--text); outline: none; flex: 1; min-width: 100px;
    }
    .tag-input::placeholder { color: var(--muted); }

    /* ── Color picker ───────────────────────────────── */
    .color-list { display: flex; flex-wrap: wrap; gap: 10px; margin-bottom: 14px; }
    ...
    .add-color-btn {
      width: 40px; height: 40px; border-radius: 50%; border: 2px dashed var(--border);
      background: #fafafa; cursor: pointer; display: flex; align-items: center;
      justify-content: center; color: var(--muted); transition: border-color .15s, color .15s;
    }
    ...
    .color-modal {
      ...
      box-shadow: 0 12px 40px rgba(0,0,0,0.18); padding: 24px; width: 320px;
    }
    ...
    .btn-cancel-modal {
      ...
      background: var(--white); font-family: var(--font); font-size: 13px; font-weight: 600; cursor: pointer;
    }
    ...
    .btn-secondary {
      ...
      cursor: pointer; color: var(--text); text-decoration: none; transition: background .15s;
    }
    .btn-secondary:hover { background: #f5f5f5; }
    .btn-primary {
      padding: 10px 24px; border-radius: 10px; border: none; background: var(--blue); color: #fff;
  ...
  <main class="main-content">
    ...
    <div class="top-bar">
      <div class="top-bar-left">
        <a href="../admin/admin-products.php" class="back-btn">
    ...
    <?php if (isset($dbConnError)): ?>
      <div class="alert alert-warn">
        ⚠ Database not connected — the form will not save until this is resolved.<br>
        <small style="opacity:.7"><?= htmlspecialchars($dbConnError) ?></small>
      </div>
    <?php endif; ?>
      ...
      <div class="form-grid">
        ...
        <div>
          ...
          <div class="card">
            ...
            <div class="form-group">
              ...
              <input class="form-input" type="text" id="product_name" name="product_name"
                     placeholder="e.g. HyperX Cloud Stinger"
          ...
          <div class="card">
            ...
            <div class="stock-grid">
              ...
              <div class="form-group" style="margin-bottom:0">
                <label class="form-label" for="low_stock_threshold">
                  Low Stock Alert At <span class="opt">(optional)</span>
                </label>
            ...
            <div class="form-group" style="margin-top:14px">
              <label class="form-label" for="stock_status">
                Stock Status <span class="req">*</span>
              </label>
              <div class="select-wrap">
                <select class="form-select" id="stock_status" name="stock_status">
                  <option value="in_stock"     <?= (($_POST['stock_status'] ?? 'in_stock') === 'in_stock')     ? 'selected' : '' ?>>In Stock</option>
          ...
          <div class="form-footer">
            <a href="../admin/admin-products.php" class="btn-secondary">Cancel</a>
        ...
        <div>
          ...
          <div class="card">
            ...
            <div class="form-group">
              ...
              <p class="section-hint">Type a tag then press Enter or comma to add it.</p>
              <div class="tags-wrap" id="tagsWrap" onclick="document.getElementById('tagInput').focus()">
                <input type="text" id="tagInput" class="tag-input" placeholder="e.g. gaming, wireless, new"/>
              </div>
...
<div class="color-modal-overlay" id="colorModalOverlay">
  <div class="color-modal">
    ...
    <div class="color-preview-box" id="colorPreviewBox" style="background:#3b82f6"></div>
    <div class="color-row">
      <input type="color" class="native-color-input" id="nativeColorPicker" value="#3b82f6"
             oninput="syncColorFromNative(this.value)"/>
      <div class="input-prefix" style="flex:1">
        <span class="input-prefix-label">#</span>
        <input type="text" id="hexInput" placeholder="3b82f6" maxlength="6"
...
function toggleDiscount(cb) {
  ...
  if (!cb.checked) document.getElementById('discounted_price').value = '';
}

// ── Image upload & preview ─────────────────────────────
let selectedFiles = [];

function previewImages(input) { handleFiles(Array.from(input.files)); }
...
function removeImage(i) { selectedFiles.splice(i, 1); renderPreviews(); }
...
// ── Color picker ───────────────────────────────────────
let colors = [], editingIndex = null, currentHex = '#3b82f6';
...
function openColorModal(index) {
  ...
  syncUI(currentHex);
  document.getElementById('colorModalOverlay').classList.add('open');
}
...
function confirmColor() {
  ...
  if (editingIndex !== null) colors[editingIndex] = entry; else colors.push(entry);
  renderColors(); closeColorModal();
}
...
document.getElementById('productForm').addEventListener('submit', function(e) {
  ...
  // Attach staged files to the real file input
